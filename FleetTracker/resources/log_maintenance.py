...
packing_brass, packing_nobrass, unitnumber, crew, maintenance_type):
    """Adds an entry to maintenance given variables in request body"""
    models.maintenance.insert({'maintenance_id': uuid.uuid4().hex, 'pump_hours': pump_hours, 'Hole': hole,
    'Treater': supervisor, 'grease_pressure': grease_pressure,
    'suction_valves': suction_valves,  'suction_seats': suction_seats, 'discharge_valves': discharge_valves,
    'discharge_seats': discharge_seats, 'suction_spring': suction_spring, 'discharge_spring': discharge_spring,
    'packing_brass': packing_brass, 'packing_nobrass': packing_nobrass, 'UnitNumber': unitnumber, 'Crew': crew, 'MaintenanceType': maintenance_type}, typecast=True)
...
class LogMaintenance(Resource):
    ...
    def post(self):
        ...
        load_data['packing_brass'], load_data['packing_nobrass'], load_data['unitnumber'], load_data['crew'], load_data['maintenance_type'])
